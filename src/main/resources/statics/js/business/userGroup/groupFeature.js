...
var vm = new Vue({
	...
	methods: {
		...
		queryCampAllTagDetail: function(campId){
			...
			ajaxFunctionPost(ctxPath + "/userGroup/h62campaigninfo/queryCampFeature", data, vm.successPro, vm.errorPro);
		},
		successPro: function(res){
			vm.campTagDetails = res.campFature.tagMap;
			vm.personCntSum = res.campFature.campPersons;
...
//	                return '群总人数： ' + params.value;
//	            }else{
//	                return params.name + 
//	                '<br/>群内符合标签的人数： ' + params.value + 
//	                '<br/>占群人数比例：' + (null == personCnt? params.value : ((params.value / personCnt) * 100).toFixed(2) + '%');
//	            }
	        	return params[0].name + 
                '<br/>群内符合标签的人数： ' + params[0].value + 
                '<br/>占群人数比例：' + (null == personCnt? params[0].value : ((params[0].value / personCnt) * 100).toFixed(2) + '%');
	        },
