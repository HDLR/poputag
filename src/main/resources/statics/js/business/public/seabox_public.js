...
/**
 * 点击查询按钮后调用该方法 搜集DIV标签searchDiv中所有type为input的输入框内容
...
function showPageForSplitExt(url, callbackFunction, currPage, pageSize, paramArray) {
	// var paramArray = new Array();// initSearchParam();
	if (paramArray == null)
		return;
	if (currPage == null)
		currPage = 1;
...
function initPagination(url, pageData, callbackFunction) {
	...
	/*
	 * paginationDivObj.empty(); var beforePageObj = $("<span
	 * class='pageIndex'>&lt;</span>"); beforePageObj.click(function(){ var
	 * activeValue = $("#paginationDiv span.active").text(); if(activeValue==1){
	 * return; } if(activeValue == '') { activeValue = 2; // next goes to page
	 * '1' } showPageForSplit(url,callbackFunction,activeValue-1,pageSize); });
	 * paginationDivObj.append(beforePageObj); var maxPageNum = 0; for(var i=0;i<pageTotalResult;i++){
	 * var spanNum = i+1; var idValue = 'span_'+spanNum; var spanObj = $("<span>"+spanNum+"</span>");
	 * if(currPage==(i+1)){ spanObj.attr("class","active currentPage"); } else{
	 * spanObj.attr("class","currentPage"); } paginationDivObj.append(spanObj);
	 ...
	 * afterPageObj.click(function(){ var activeValue = $("#paginationDiv
	 * span.active").text(); if(activeValue==maxPageNum){ return; }
	 * if(activeValue == '') { activeValue = 0; // next goes to page '1' }
 ...
 * 
 * @param param
 *            请求后台controller时带过去的参数,类型为Array{id:paramName,value:paramValue}
 *            如:param = new Array(); param[0]={id:'testParam',value:'123456'}
 * @param url
 *            后台Controller地址。如想要请求http://localhost:8080/TEST/com/testcontroller/test.do,url应为:/com/testcontroller/test.do
 */
...
function checkNotNull() {
	...
	$.each(allNotNullObjs, function(i, notNullObj) {
		if ($(notNullObj).val() != null && $(notNullObj).val() != '') {// do
			// nothing}
	...
	if (nullObjNames != "") {
		...
		var msgText = "<div style='width:90%;height:90%;margin-left:5%;margin-right:5%;margin-top:5%;line-height:20px;text-align:center;'>"
				+ nullObjNames + "</div>";
...
function checknum(String) {
	...
	var i;
	var c;
	for (i = 0; i < Letters.length; i++) { // Letters.length()
		c = Letters.charAt(i);
...
/**
 * 对列表可排序字段点击时, 更新排序图标 需要请求排序的页面存在有 id="searchDiv" 的 组件 比如: <div id="searchDiv"
 * ... >
 * 
 * @param orderColumn
 */
...
/**
 * 显示文字截取 str源字符串 subIndex:截取位置
 */
function getSubstr(str, subIndex) {
	if (isNull(subIndex)) {
		subIndex = 15
	}
	...
	subIndex = subIndex > 3 ? subIndex - 1 : 0;
	str = str.substr(0, subIndex) + "......";
	return str;
}
...
function isNull(str) {
	...
	// console.log(((str+"1")=="01")+","+str+","+(str+"1"));
	if (str == null || str == '' || str == undefined) {
		if ((str + "1") == "01") {// 防止字符串为0时,判断为空
			return false;
		}
		return true;
	}
...
/**
 * 判断输入是否为数字 true为数字 false符数字
 */
function isNum(str) {
	return !isNaN(str);
}

function isExistsById(id, url) {
...
function commSave(url, formId, saveCallbak) {
	var isOk = false;
	$.ajax({
		type : "POST",
		url : url,
...
function getUpDateKeys(PRIMARY_KEY) {
	var key = "";
	for ( var i in PRIMARY_KEY) {
		if (key.length > 0) {
			key = key + "#" + i + ":" + PRIMARY_KEY[i]
		} else {
			key = i + ":" + PRIMARY_KEY[i]
		}
	}
