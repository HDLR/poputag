package com.eastern.jinxin.business.api.interceptor;
...
public class AuthorizationInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //查询token信息
        TbTokenEntity TbToken = tokenService.queryByToken(token);
        if(TbToken == null || TbToken.getExpireTime().getTime() < System.currentTimeMillis()){
            throw new RRException("token失效，请重新登录");
        }

        //设置userId到request里，后续根据userId，获取用户信息
        request.setAttribute(LOGIN_USER_KEY, TbToken.getUserId());
